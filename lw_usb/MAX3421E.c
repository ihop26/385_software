...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	XSpi_Transfer(&SpiInstance, SendBuf, NULL, 2);
	//write val via SPI

	//read return code from SPI peripheral (see Xilinx examples) 
	//if return code != 0 print an error
	if(Status != 0){
		xil_printf ("SPI device reg write fail %d", Status);
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	while(numbytes<nbytes){
		...
		//if return code != 0 print an error
		SendBuf[1] = *(data+numbytes);
		Status = XSpi_Transfer(&SpiInstance, SendBuf , NULL, 2);
		if(Status != 0){
			xil_printf ("SPI device reg multi write fail %d", Status);
		}
		numbytes++;
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//psuedocode:
		...
		XSpi_Transfer(&SpiInstance, SendBuf, NULL, 1);

		Status = XSpi_Transfer(&SpiInstance, SendBuf, RecBuf, 2);
		if(Status != 0){
			xil_printf ("SPI device reg read fail %d", Status);
		}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	while(numbytes<nbytes){
		...
		Status = XSpi_Transfer(&SpiInstance, SendBuf , RecBuf, 2);
		if(Status != 0){
			xil_printf ("SPI device reg multi read fail %d", Status);
		}
		*(data+numbytes) = RecBuf[1];
		numbytes++;
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0x0);
	return (data + nbytes);
